    ...
    <div class="sticky top-0 z-50 flex justify-between py-5 px-4 lg:px-14 bg-white shadow-sm">
      <div class="flex gap-10 w-full">
        ...
        <div id="menu" class="hidden lg:flex flex-col lg:flex-row lg:items-center lg:gap-10 w-full lg:w-auto mt-5 lg:mt-0">
          <ul class="flex flex-col lg:flex-row items-start lg:items-center gap-4 font-medium text-base w-full lg:w-auto">
            ...
            <a href="/src/politik.php" class="hover:text-primary">Politik</a>
      ...
      <div class="hidden lg:flex items-center gap-2 mt-4 lg:mt-0 w-full lg:w-auto relative">
        <div class="relative w-full lg:w-auto">
          <form action="hasilpercarian.php" method="GET" class="flex">
        ...
        <?php if ($isLoggedIn): ?>
          <!-- Jika sudah login -->
          <div class="flex items-center gap-2">
            <span class="text-sm text-slate-600">Halo, <?php echo htmlspecialchars($userName); ?></span>
            <a href="logout.php" class="bg-primary px-8 py-2 rounded-full text-white font-semibold h-fit text-sm lg:text-base">
              Logout
            </a>
          </div>
        <?php else: ?>
          <!-- Jika belum login -->
          <a href="login.php" class="bg-primary px-8 py-2 rounded-full text-white font-semibold h-fit text-sm lg:text-base">
            Masuk
          </a>
        <?php endif; ?>
      </div>
